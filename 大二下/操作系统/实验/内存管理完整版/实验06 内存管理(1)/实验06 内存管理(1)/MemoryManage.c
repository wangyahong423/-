...
int MallocMemory(PCB *pe, SqList *pPartTable,int i)//分配空间，修改分区说明表信息，	i表示第几个
{
	///////////   以下需要补充    /////////////
...
void InitAllocation(PCBList PCBdata, PartTable partTable)//初始分配，此时内存为空
{
	///////////   以下需要补充    /////////////
	LinkList p = PCBdata->Next;
	int pos;
	while(p) {
...
void FreeMemory(int pos, SqList *pPartTable)//释放空间，pos分区说明表第几个控制块
{
	///////////   以下需要补充    /////////////
	strcpy(pPartTable->elem[pos].Name,"");//释放内存，变成空的就好了
